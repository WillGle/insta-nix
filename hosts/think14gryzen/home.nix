{ ... }:
{
  ...
  # Waybar's own bluetooth module now occupies the bar, so blueman's tray icon
  # is a second bluetooth indicator sitting right next to it. Drop the StatusIcon
  # plugin, which is what spawns blueman-tray and registers /org/blueman/sni.
  # A "!" prefix disables a plugin (blueman/main/PluginManager.py); a bare name
  # enables one that does not autoload, which is why NetUsage has to stay listed
  # or it silently switches off with this write.
  #
  # Only ShowConnected hard-depends on StatusIcon, and it just drives that
  # icon's tooltip — it fails to load and is caught. KillSwitch and PowerManager
  # both guard their use behind `if "StatusIcon" in Plugins.get_loaded()`.
