...
in
{
  home = {
    ...
    activation = {
      # hyprland.conf sources this file, and toggle_touchpad.sh rewrites it. Seed
      # it here so the source never dangles on a fresh machine; it must stay
      # writable, hence an activation script rather than an xdg.configFile symlink.
      ...
      # Seed the user-editable category map from the packaged default, then fold
      # in any new default categories on every rebuild without losing local edits.
